...
import { intermediate2024 } from '../data/2024/intermediate2024'
...
const seniorMaterial = intermediate2024
...
const handleSearch = () => {
  ...
  // search for reference (with number)
  // only enter if search contains a digit
  if (query.search(/\d/) > -1) {
    let queryBook = ''
    let queryChapter = 0
    let queryVerse = 0
    if (query.split(/[ .,:;]+/gi).length < 2) {
      return
    }
    if (query.split(/[ .,:;]+/gi).length == 2) {
      if ((query.split(/[ .,:;]+/gi)[0]).search(/\d/) == -1) {
        queryBook = query.split(/[ .,:;]+/gi)[0]
        queryChapter = query.split(/[ .,:;]+/gi)[1]
      } else {
        queryChapter = query.split(/[ .,:;]+/gi)[0]
        queryVerse = query.split(/[ .,:;]+/gi)[1]
      }
    }
    if (query.split(/[ .,:;]+/gi).length == 3) {
      if ((query.split(/[ .,:;]+/gi)[1]).search(/\d/) == -1) {
    ...
    if (query.split(/[ .,:;]+/gi).length > 3) {
      ...
      queryVerse = queryArray[queryArray.length - 1]
    }
    setIsReference(true)
